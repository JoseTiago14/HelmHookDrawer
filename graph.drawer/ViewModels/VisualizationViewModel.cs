...
namespace graph.drawer.ViewModels
{
    ...
    public class VisualizationViewModel
    {
        ...
        public VisualizationViewModel(IFlow flow)
        {
            ...
            PreInstalls = flow.Bind<IEnumerable<ResourceViewModel>, ParsedResult>(result => result.PreInstalls.Select(r => new ResourceViewModel(r)), Disposables);
            Installs = flow.Bind<IEnumerable<ResourceViewModel>, ParsedResult>(result => result.Installs.Select(r => new ResourceViewModel(r)), Disposables);
            PostInstalls = flow.Bind<IEnumerable<ResourceViewModel>, ParsedResult>(result => result.PostInstalls.Select(r => new ResourceViewModel(r)), Disposables);
        }
        ...
        private Dictionary<ParseState, string> ErrorMessages => new Dictionary<ParseState, string> {
